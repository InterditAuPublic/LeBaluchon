...
class CurrencyConverter {
    ...
    var exchangeRate: Double = 1.091643 // default : 0.0
    
    // call the API to get all symbols and their names (e.g. USD: United States Dollar) and store them in a dictionary
    ...
    func convert(amount: Double, completion: @escaping (Double?) -> Void) {
        ...
        if self.exchangeRate != 0.0 {
            completion(amount * self.exchangeRate)
        } else {
...
//            let usdAmount = amount * exchangeRate / usdRate
    ...
    func convertAmountWithExchangeRate(amount: Double) -> Double {
